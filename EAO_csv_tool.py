...
st.set_page_config(page_title="EAO_csv_tool", page_icon=":tiger:")
st.title(":tiger: EAO_csv_tool")
...
if not st.session_state['csv_path']:
    st.session_state['csv_path'] = st.text_input('Enter the path to csv file here: ')

if 'langchain_messages' not in st.session_state:
    st.session_state['langchain_messages'] = []
   
# Replace single quotes with double quotes and 'False' with 'false' in the JSON string
def preprocess_json(json_str,alias_name):
...
def extract_metrics(question,document):
    ...
    metric_extractor_template = """You are a metric extractor and can extract the necessary metrics from the user question based on the context provided which is a documentation containing definitions of all metrics.
             ...
             2) Always return ‘queryID’, ‘Operator’ and 'query_alias' into the list regardless of the need. 
...
df[json_stats_column] = df.apply(lambda row: preprocess_json(row[json_stats_column], row[query_alias_column]), axis=1)
for _, row in df.iterrows():
    alias_value = row['query_alias']
    json_str = row[json_stats_column]
    output_file_path = os.path.join('Actual_JSONs_Holder', f"{alias_value}.json")
    with open(output_file_path, 'w') as outfile:
        outfile.write(json_str)

query_aliases = df[query_alias_column].to_list()
input_folder = 'Actual_JSONs_Holder'
...
# clean_output_folder(input_folder)